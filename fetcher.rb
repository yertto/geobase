...
module Fetcher
  ...
  def _get_cache_file_and_data(url, cache_file=nil)
    ...
    data = if cache_file.exist?
      STDERR.puts "Reading data from #{cache_file} ..." if ENV['DEBUG']
      cache_file.read
    else
      cache_file.open('w') { |f|
        data = open(url).read
        STDERR.puts "Caching data to #{cache_file} ..." if ENV['DEBUG']
        f.write(data)
        (STDERR.puts "Sleeping for #{SLEEP_TIME} seconds ..."; sleep SLEEP_TIME) if ENV['DEBUG']
        data
      }
    end
    [cache_file, data]
